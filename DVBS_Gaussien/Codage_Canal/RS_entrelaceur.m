...
%% Ajout du code Reed Salomon + Entrelaceur %%%%%%%%%%%%%%
...
% Entrelaceur
bits_RS = convintrlv(bits_RS,8,3);
...
for k = 1:length(Eb_sur_N0)
    ...
    % Desentrelaceur
    % HARD
    bits_decode_hard_des = convdeintrlv(bits_decode_hard, 8, 3);
    bits_desentrelace_hard = circshift(bits_decode_hard_des, -8*3*7);

    % SOFT
    bits_decode_soft_des = convdeintrlv(bits_decode_soft, 8, 3);
    bits_desentrelace_soft = circshift(bits_decode_soft_des, -8*3*7);

    % Decodeage Reed-Salomon RS(204,188)
    rs_decoder = comm.RSDecoder(Nrs,Krs);

    % HARD
    bits_decode_RS_hard = step(rs_decoder, bits_desentrelace_hard.');

    % SOFT
    bits_decode_RS_soft = step(rs_decoder, bits_desentrelace_soft.');
...
hold on;
plot(Eb_sur_N0, mat_TEB_hard, 'r', 'DisplayName', 'TEB_HARD_entrelaceur');
hold off;
