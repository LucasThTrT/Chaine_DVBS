...
bits_code = convenc(bits, trellis, P);  %Expliquer 
...
for k = 1:length(Eb_sur_N0)
    ...
    mat_TEB_hard(k) = sum(bits_decode_hard ~= bits) / nb_bits;
    mat_TEB_soft(k) = sum(bits_decode_soft ~= bits) / nb_bits;
    
end

% Tracer du TEB avec décodage de viterbi soft et hard sur le meme graphique
figure;
